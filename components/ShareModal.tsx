...
export const ShareModal: React.FC = () => {
  ...
  const loadKnownPeers = () => { try { const known = JSON.parse(localStorage.getItem('neuro_known_peers') || '[]'); setKnownPeers(known); return known; } catch (e) { return []; } };
  ...
  useEffect(() => {
    const initPeer = async () => {
      try {
        ...
        peer.on('open', (id: string) => { setMyPeerId(id); setConnectionStatus('Online'); QRCode.toDataURL(JSON.stringify({id, name: userProfile.name})).then(setQrCodeUrl); loadKnownPeers(); setTimeout(connectToKnownPeers, 1500); });
        peer.on('connection', handleNewConnection);
        peerRef.current = peer;
      } catch (e) { console.error(e); }
    };
    initPeer();
    return () => { if (peerRef.current) peerRef.current.destroy(); stopScanner(); };
  }, []);
  ...
  const removePeer = (peerId: string) => { connectionsMap.current.delete(peerId); setPeers(prev => prev.filter(p => p.id !== peerId)); };
  
  const handleIncomingData = (conn: DataConnection, data: any) => {
      const peerId = conn.peer;
      if (data.type === 'handshake') {
          let displayName = data.name || 'Unknown';
          if (displayName === userProfile.name) displayName = `${displayName} (${data.id?.substring(0, 4)})`;
  ...
  const connectToPeer = (id: string) => { if (!peerRef.current || !id || connectionsMap.current.has(id)) return; handleNewConnection(peerRef.current.connect(id)); };

  const sendMessage = () => {
      if (!textInput.trim()) return;
      if (textInput === ADMIN_CODE) { enableAdminMode(); alert("Admin Mode Activated"); setTextInput(''); return; }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 backdrop-blur-md animate-in fade-in">
        ...
        {showLargeQR && qrCodeUrl && (
            <div className="absolute inset-0 z-[70] bg-black/95 flex flex-col items-center justify-center p-6 animate-in zoom-in-95">
                ...
                <button onClick={() => setShowLargeQR(false)} className="mt-8 bg-slate-800 hover:bg-slate-700 text-white px-8 py-3 rounded-full font-bold border border-slate-700 transition-all">Close</button>
            </div>
        )}
            ...
            {shareViewMode === 'transfer' && (
                <div className="flex-1 flex flex-col">
                    <div className="p-6 bg-slate-950 border-b border-slate-800 flex flex-col items-center justify-center relative">
                         <div className="relative mb-2"><div className={`w-16 h-16 rounded-full flex items-center justify-center ${peers.length > 0 ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-slate-500 animate-pulse'}`}><Icons.Radar size={32} className={peers.length > 0 ? '' : 'animate-spin-slow'} /></div>{peers.length > 0 && <div className="absolute -bottom-1 -right-1 bg-green-500 text-slate-900 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{peers.length}</div>}</div>
                         ...
                         {transferTab === 'dashboard' && <div className="flex space-x-3 w-full max-w-xs"><button onClick={startScanner} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-2 rounded-lg flex items-center justify-center space-x-2 text-sm font-bold shadow-lg"><Icons.Scan size={16} /> <span>Scan</span></button><button onClick={() => setShowLargeQR(true)} className="flex-1 bg-slate-800 text-white py-2 rounded-lg flex items-center justify-center space-x-2 text-sm font-bold border border-slate-700"><Icons.QrCode size={16} /> <span>My Code</span></button></div>}
                    ...
                    {transferTab === 'dashboard' && <div className="flex-1 overflow-y-auto p-4 space-y-4"><div className="grid grid-cols-2 gap-4"><button onClick={() => setTransferTab('files')} className="bg-slate-800 p-4 rounded-xl flex flex-col items-center justify-center hover:bg-slate-750 transition-colors border border-slate-700"><div className="w-12 h-12 bg-blue-500/20 text-blue-400 rounded-full flex items-center justify-center mb-2"><Icons.Send size={24} /></div><span className="text-sm font-bold text-slate-200">Send Files</span></button><div className="bg-slate-800 p-4 rounded-xl flex flex-col items-center justify-center border border-slate-700 opacity-60"><div className="w-12 h-12 bg-green-500/20 text-green-400 rounded-full flex items-center justify-center mb-2"><Icons.Download size={24} /></div><span className="text-sm font-bold text-slate-200">Ready to Receive</span></div></div>{knownPeers.length > 0 && <div><h3 className="text-xs font-bold text-slate-500 uppercase mb-3 px-1">Saved Devices</h3><div className="space-y-2">{knownPeers.map(p => { const isOnline = peers.some(peer => peer.id === p.id); return (<div key={p.id} onClick={() => connectToPeer(p.id)} className="bg-slate-800 p-3 rounded-xl flex items-center justify-between border border-slate-700/50 hover:bg-slate-750 cursor-pointer"><div className="flex items-center space-x-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${isOnline ? 'bg-green-500/20 text-green-400' : 'bg-slate-700 text-slate-400'}`}>{isOnline ? <Icons.Wifi size={18} /> : <Icons.Phone size={18} />}</div><div><p className="text-sm font-bold text-slate-200">{p.name}</p><p className="text-[10px] text-slate-500">{isOnline ? 'Connected' : 'Tap to Reconnect'}</p></div></div>{!isOnline && <Icons.ChevronRight size={16} className="text-slate-600" />}</div>);})}</div></div>}<div className="bg-slate-800/30 border border-slate-800 rounded-xl p-4"><h3 className="text-xs font-bold text-slate-500 uppercase mb-2">Connect via Code</h3><div className="flex space-x-2"><input value={connectIdInput} onChange={(e) => setConnectIdInput(e.target.value.toUpperCase())} placeholder="ENTER REMOTE ID" className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white font-mono uppercase focus:border-blue-500 outline-none" /><button onClick={() => connectToPeer(connectIdInput)} disabled={!connectIdInput} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-bold disabled:opacity-50">Connect</button></div></div></div>}
                                 ...
                                 <div key={msg.id} className={`flex ${msg.senderId === myPeerId || msg.senderId === 'me' ? 'justify-end' : 'justify-start'}`}>
                                     {msg.isSystem ? <span className="bg-slate-800 text-slate-500 text-[10px] px-2 py-1 rounded-full">{msg.content}</span> : (
                                         <div className="relative group">
                                             ...
                                             {userProfile.isAdmin && <button onClick={() => handleDeleteMessage(msg.id)} className="absolute -top-2 -right-2 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg" title="Delete Message"><Icons.Trash size={12} /></button>}
                                         </div>
                                     )}
                                 </div>
                             ))}
